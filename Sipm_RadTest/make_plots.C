void make_plots(){
	...
	h1a->GetXaxis()->SetTitle("Energy deposited in SiPM [MeV]");h1a->GetXaxis()->CenterTitle();
        ...
        TH1 *h1b = new TH1D("h1b","For 55 MeV Protons",100,-0.1,6);
	h1b->SetLineColor(kGreen);h1b->SetLineWidth(3);
	h1b->GetXaxis()->SetTitle("Energy deposited in SiPM [MeV]");h1b->GetXaxis()->CenterTitle();
	...
	t1->Draw("SipmHits.energy*1000.>>h1b","fabs(SipmHits.position.z-23.775)<0.01");

	TCanvas *c1 = new TCanvas("c1");
	c1->SetLogy();
	h1->Draw();h1a->Draw("same");h1b->Draw("same");
	...
	TPaveText* tex1b = new TPaveText(0.65,0.55,0.85,0.75,"NDCNB");
	tex1b->AddText("3^{rd} SiPM");tex1b->SetTextColor(kGreen);
	tex1b->SetFillStyle(4000);tex1b->SetTextFont(63);tex1b->SetTextSize(20);
	tex1b->Draw();
